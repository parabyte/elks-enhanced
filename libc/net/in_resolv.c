...
static char *read_resolv_cfg(char *buf, size_t buflen)
{
	FILE *fp;
	char line[80];
	char *p;
...
ipaddr_t in_resolv(const char *hostname, char *server)
{
	...
	sighandler_t old;
	char buf[256];
	char resolv_server[32];

	if (server == NULL)
		server = getenv(DNS_ENV);
	if (server == NULL)
		server = read_resolv_cfg(resolv_server, sizeof(resolv_server));
	if (server == NULL)
		server = DEFAULT_DNS;
