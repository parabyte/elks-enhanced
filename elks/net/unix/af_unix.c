...
static int unix_sendto(struct socket *sock, void *buff, int len, int nonblock,
		       unsigned flags, struct sockaddr *addr, size_t addrlen)
{
    if (addr != NULL || addrlen != 0)
	return -EISCONN;
    return unix_send(sock, buff, len, nonblock, flags);
}

static int unix_recvfrom(struct socket *sock, void *buff, int len, int nonblock,
			 unsigned flags, struct sockaddr *addr, int *addrlen)
{
    if (addr != NULL || addrlen != NULL)
	return -EISCONN;
    return unix_recv(sock, buff, len, nonblock, flags);
}
