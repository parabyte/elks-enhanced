...
int sys_listen(int fd, int backlog)
{
	...
	printk("sys_listen reject sock %x state %d flags %x type %d\n",
	    sock, sock->state, sock->flags, sock->type);
	return -EINVAL;
    }
    ...
    ops = sock->ops;
    if (ops && ops->listen) {
	backlog = ops->listen(sock, backlog);
	if (backlog < 0) {
	    printk("sys_listen ops reject sock %x err %d state %d flags %x\n",
		sock, backlog, sock->state, sock->flags);
	    return backlog;
	}
    }
...
int sys_ksendto(int fd, void *ubuf, int size, struct socket_sendto_args *uargs)
{
    struct socket *sock;
    struct file *file;
    ...
    if (uargs) {
	if (verify_area(VERIFY_READ, uargs, sizeof(args)))
	    return -EFAULT;
	memcpy_fromfs(&args, uargs, sizeof(args));
	if (args.addrlen > sizeof(args.addr))
	    return -EINVAL;
    }

    return sock->ops->sendto(sock, ubuf, size, (file->f_flags & O_NONBLOCK),
	args.flags, args.addrlen ? &args.addr : NULL, args.addrlen);
}

int sys_krecvfrom(int fd, void *ubuf, int size, struct socket_recvfrom_args *uargs)
{
    struct socket *sock;
    struct file *file;
    ...
    memset(&args, 0, sizeof(args));
    if (uargs) {
	if (verify_area(VERIFY_WRITE, uargs, sizeof(args)))
	    return -EFAULT;
	memcpy_fromfs(&args, uargs, sizeof(args));
	if (args.addrlen > sizeof(args.addr))
    ...
    err = sock->ops->recvfrom(sock, ubuf, size, (file->f_flags & O_NONBLOCK),
	args.flags, uargs ? &args.addr : NULL, uargs ? (int *)&args.addrlen : NULL);
    if (err >= 0 && uargs) {
	if (verified_memcpy_tofs(uargs, &args, sizeof(args)) < 0)
	    return -EFAULT;
    }
    return err;
}
